...
createFeSectionWidget::createFeSectionWidget(QWidget *parent) :
    QWidget(parent)
{

        closeButton = new QPushButton("Close",this);
        startButton=new QPushButton("Apply",this);
        header = new QLabel("FE-Sections: ",this);
        ...
        materialHeader = new QLabel("Material:",this);
        ...
        vLayout->addWidget(materialHeader);
        vLayout->addWidget(materialComboBox);
    ...
    void createFeSectionWidget::startClicked()
    {
        if (name->text()=="")
        {
            emit sendMessage(QString("A name must be specified! Section Creation aborted!"));
            return;
        }
        if (materialComboBox->currentText()=="")
        {
            emit sendMessage(QString("A material must be specified! Section Creation aborted!"));
            return;
        }
        emit startSectCreation(name->text(),targetSetComboBox->currentText(),materialComboBox->currentText(),1,1);
    }
